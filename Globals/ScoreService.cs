...
public partial class ScoreService : Node {
  [Signal]
  public delegate void ScoreChangedEventHandler(int totalScore);
  ...
  private void _SyncMultiplier() {
    ...
    foreach (var effectVariant in inventory.GetActiveEffects()) {
      ...
      if (effect.ContainsKey("kind") &&
          effect["kind"].AsString() == "score_multiplier" &&
          effect.ContainsKey("factor"))
  ...
  public int TotalStrokes =>
      _rounds.Sum(r => r.SmallStrokes + r.MediumStrokes + r.LargeStrokes) +
  ...
  public void StartRound() { _current = default; }
  ...
  public void LoadFromSave(Godot.Collections.Dictionary saveData) {
    ...
    var restored = new RoundData {
      ...
      SmallStrokes =
          saveData.ContainsKey("strokes") ? saveData["strokes"].AsInt32() : 0,
    };
  ...
  // Docks points from the current round's score (pause penalty). Clamped at 0
  // so a penalty can never eat into previously banked rounds. Emits
  // ScoreChanged so the HUD reflects the drain live.
  ...
  GetRoundBreakdowns() {
    ...
    foreach (var round in _rounds) {
      var breakdown = new Godot.Collections.Dictionary {
        ["score"] = round.Score,          ["small"] = round.SmallStrokes,
        ["medium"] = round.MediumStrokes, ["large"] = round.LargeStrokes,
