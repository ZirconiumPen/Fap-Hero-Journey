...
// Format: L0XXXXIDDDD\n  — linear axis 0, position 0-9999, interpolate over
// DDDD ms.
public partial class SerialDeviceService : Node {
  ...
  public bool Connect(string portName, int baudRate = DefaultBaudRate) {
    ...
    try {
      _port = new SerialPort(portName, baudRate) {
        NewLine = "\n",   WriteTimeout = 100, ReadTimeout = 100,
  ...
  // position: 0.0-1.0, durationMs: how long the device should take to reach the
  // target. TCode expects 0-9999 pos.
  ...
  // Send a command to any named T-code axis (e.g. "L1", "L2", "R0", "R1",
  // "R2"). Uses the same interpolated-linear format as SendLinear. position:
  // 0.0–1.0, durationMs: travel time in ms. TCode expects 0-9999 pos.
  ...
  public void SendVibrate(double intensity) {
    ...
    int intensityTicks =
        Math.Clamp((int)Math.Round(intensity * 9999.0), 0, 9999);
