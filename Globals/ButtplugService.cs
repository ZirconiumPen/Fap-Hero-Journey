...
public partial class ButtplugService : Node {
  ...
  [Signal]
  public delegate void DeviceAddedEventHandler(string name, int index);
  [Signal]
  ...
  public async void ConnectToIntiface(string address) {
    ...
    try {
      ...
      Callable.From(() => EmitSignal(SignalName.ErrorOccurred, e.Message))
          .CallDeferred();
    }
  }
  ...
  public int GetSelectedDeviceIndex() {
    ...
    string selectedName =
        GetNode("/root/SettingsService").Call("get_selected_device").AsString();
  ...
  // Returns the number of independent vibration channels (actuators) on the
  // device. Most single-motor devices return 1; dual-motor devices (e.g.
  // We-Vibe Sync, Lovense Nora/Max 2) return 2.
  ...
  public async void SendVibrateChannel(int deviceIndex, int channelIndex,
                                       double intensity) {
    var device = GetDeviceAt(deviceIndex);
    if (device == null)
      return;
    ...
    try {
      ...
      var feature = device.GetFeaturesWithOutput(OutputType.Vibrate)
                        .ElementAtOrDefault(channelIndex);
      if (feature == null) {
      ...
      await feature.RunOutputAsync(DeviceOutput.Vibrate.Percent(intensity),
                                   default);
    } catch (Exception e) {
  ...
  public async void SendLinear(int deviceIndex, uint durationMs,
                               double position) {
    var device = GetDeviceAt(deviceIndex);
    if (device == null)
      return;
    ...
    try {
      await device.RunOutputAsync(
          DeviceOutput.PositionWithDuration.Percent(position, durationMs));
    } catch (Exception e) {
  ...
  private void OnDeviceAdded(object sender, DeviceAddedEventArgs e) {
    ...
    Callable
        .From(() => EmitSignal(SignalName.DeviceAdded, deviceName, deviceIndex))
        .CallDeferred();
  }

  private void OnDeviceRemoved(object sender, DeviceRemovedEventArgs e) {
    int deviceIndex = (int)e.Device.Index;
    Callable.From(() => EmitSignal(SignalName.DeviceRemoved, deviceIndex))
        .CallDeferred();
  }
