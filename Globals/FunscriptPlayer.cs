...
public partial class FunscriptPlayer : Node {
  ...
  private int _vibChannelCount =
      0; // resolved in ResolveOutput(); 0 for non-vibrators
  ...
  // open so the device doesn't sit idle. Independent of _playing / the
  // funscript.
  ...
  private double _mirrorClockMs =
      double.NaN; // last clock the blend advanced from
  ...
  public void LoadFunscript(string path) {
    ...
    using var funscriptFile =
        FileAccess.Open(absPath, FileAccess.ModeFlags.Read);
    ...
    var rawActions = funscript.ContainsKey("actions")
                         ? funscript["actions"].AsGodotArray()
  ...
  /// Returns the V-motion beats as Vector2(timeMs, depth 0-100) for the beat
  /// bar.
  ...
  // Fixed duration used only when parking unloaded secondary axes at round
  // start.
  ...
  public void SetVibeIntensity(int percent) => _vibeIntensity =
      Math.Clamp(percent, 0, 100) / 100f;

  /// Live-update the max stroke speed cap from Options (units/sec, 0 = off).
  public void SetMaxStrokeSpeed(int unitsPerSec) => _maxStrokeSpeed =
      Math.Max(0, unitsPerSec);
  ...
  public void ClearAxisScripts() { _axes.Clear(); }
  ...
  public void LoadVibScript(int channel, string path) {
    ...
    if (file == null) {
      GD.PrintErr(
          $"FunscriptPlayer: cannot open vib script ch{channel}: {path}");
      return;
    }
    var parser = new Json();
    if (parser.Parse(file.GetAsText()) != Error.Ok) {
      GD.PrintErr(
          $"FunscriptPlayer: JSON parse error in vib script ch{channel}: {path}");
      return;
    }
    var funscript = parser.Data.AsGodotDictionary();
    var rawActions = funscript.ContainsKey("actions")
  ...
  public void ClearVibScripts() { _vibScripts.Clear(); }
  ...
  public void Stop() {
    ...
    _easing = false;
    _fillerActive =
        false; // cancel any storyboard filler that may still be running
  ...
  // half-cycle speed. Respects the device range clamp but not inventory
  // effects. lo/hi are in the same 0–100 scale as funscript positions. Live
  // setter for filler parameters. Used by the Options overlay so a user
  ...
  // so the device always approaches at a consistent speed regardless of gap
  // size. Skipped entirely for vibrators — intensity jumps are not jarring the
  // way sudden linear strokes are, so no ease is needed.
  private void _StartEaseIn() {
    ...
    _easeDurationMs =
        Math.Clamp(gap / EaseSpeedUnitsPerMs, EaseMinMs, EaseMaxMs);
  ...
  // Call this each frame from GameLoop to keep funscript in sync with the video
  // clock. Only updates _positionMs — _Process is responsible for dispatching
  // due actions.
  ...
  public override void _Process(double delta) {
    ...
    if (_playing) {
      // When synced to a video clock, SyncTo already set _positionMs this
      // frame. Only accumulate delta in free-running mode (no video /
      // funscript-only).
      if (_syncedThisFrame)
      ...
      // ease-in as L0 so all axes blend in together from neutral at round
      // start.
      if (_outputMode == OutputMode.Serial) {
        var serial = _serial;
        if (serial != null && serial.SerialConnected) {
          ...
          foreach (var multiaxis in _axes) {
            ...
            while (state.Index < state.Actions.Count) {
              ...
              if (idx + 1 < state.Actions.Count) {
                ...
                uint durMs =
                    (uint)Math.Max(1, (int)(state.Actions[idx + 1].AtMs -
                                            state.Actions[idx].AtMs));
      ...
      if (_outputMode == OutputMode.Buttplug && _isLinearDevice == false &&
          _vibScripts.Count > 0) {
        var bpVib = _buttplug;
        if (bpVib != null && bpVib.BpConnected && _deviceIndex >= 0) {
          ...
          foreach (var vibEntry in _vibScripts) {
            ...
            while (vstate.Index < vstate.Actions.Count) {
              ...
              double intensity =
                  Math.Clamp(vstate.Actions[vstate.Index].Pos / 100.0, 0.0,
                             1.0) *
                  _vibeIntensity;
  ...
  // Send a single linear command to the device for the current filler
  // direction.
  ...
  private void ResolveOutput() {
    ...
    _homePosition =
        Math.Clamp(_settings.Call("get_home_position").AsInt32(), 0, 100);
    _homeEaseMs =
        (uint)Math.Max(50, _settings.Call("get_home_ease_ms").AsInt32());
    ...
    _vibeIntensity =
        Math.Clamp(_settings.Call("get_vibe_intensity").AsInt32(), 0, 100) /
        100f;
    _maxStrokeSpeed =
        Math.Max(0, _settings.Call("get_max_stroke_speed").AsInt32());
    ...
    if (_outputMode == OutputMode.Serial) {
      ...
      if (bp != null) {
        ...
        _isLinearDevice =
            _deviceIndex >= 0 && bp.DeviceSupportsLinear(_deviceIndex);
        ...
        _vibChannelCount = (_isLinearDevice == false && _deviceIndex >= 0)
                               ? bp.GetVibrationChannelCount(_deviceIndex)
                               : 0;
      }
    }
  ...
  private void SendCommand(int index) {
    ...
    int nextPos = index + 1 < _actions.Count ? TransformPos(index + 1, effects)
                                             : currentPos;
    ...
    if (_easing && _isLinearDevice != false) {
      ...
      currentPos = (int)Math.Round(_homePosition +
                                   (currentPos - _homePosition) * smooth);
      nextPos =
          (int)Math.Round(_homePosition + (nextPos - _homePosition) * smooth);
    ...
    if (_outputMode == OutputMode.Serial) {
      ...
      uint durationMs = (uint)Math.Max(
          1, (int)(_actions[index + 1].AtMs - _actions[index].AtMs));
    ...
    if (_isLinearDevice == true) {
      ...
      // Skip if vib scripts are loaded — per-channel dispatch runs in
      // _Process().
  ...
  private void UpdateMirrorBlend(Godot.Collections.Array effects) {
    ...
    double dt =
        double.IsNaN(_mirrorClockMs) ? 0.0 : _positionMs - _mirrorClockMs;
    ...
    _mirrorBlend =
        Mathf.MoveToward(_mirrorBlend, target, (float)(dt / MirrorEaseMs));
  }
  ...
  private int TransformPos(int index, Godot.Collections.Array effects) {
    ...
    foreach (var effect in effects) {
      ...
      if (effectProp.ContainsKey("kind") &&
          effectProp["kind"].AsString() == "scale" &&
    ...
    if (!Mathf.IsEqualApprox(scaleFactor, 1f)) {
      ...
      float next =
          MirrorOne(_actions[Math.Min(_actions.Count - 1, index + 1)].Pos);
    ...
    foreach (var effect in effects) {
      var effectProp = effect.AsGodotDictionary();
      if (effectProp.ContainsKey("kind") &&
          effectProp["kind"].AsString() == "clamp") {
        float minV =
            effectProp.ContainsKey("min") ? effectProp["min"].AsSingle() : 0f;
        float maxV =
            effectProp.ContainsKey("max") ? effectProp["max"].AsSingle() : 100f;
