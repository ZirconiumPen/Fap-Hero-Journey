...
public partial class GameState : Node {
  ...
  //   { "type": "fork_choice", "fork_title": string, "path_name": string,
  //   "path_index": int, "depth": int } { "type": "round",       "data":
  //   Dictionary, "depth": int }
  ...
  private static List<Dictionary> BuildSequence(Dictionary journeyData) {
    ...
    var rounds = journeyData.ContainsKey("rounds")
                     ? journeyData["rounds"].AsGodotArray()
                     : new Array();
    foreach (var roundVariant in rounds) {
      var roundData = roundVariant.AsGodotDictionary();
      int order =
          roundData.ContainsKey("order") ? roundData["order"].AsInt32() : 0;
      items.Add((order * 3,
                 new Dictionary { ["type"] = "round", ["data"] = roundData }));
    }

    var shops = journeyData.ContainsKey("shops")
                    ? journeyData["shops"].AsGodotArray()
                    : new Array();
    foreach (var shopVariant in shops) {
      ...
      if (shopVariant.VariantType == Variant.Type.Dictionary) {
        ...
        afterOrder = shopData.ContainsKey("after_order")
                         ? shopData["after_order"].AsInt32()
                         : 0;
      } else {
      ...
      items.Add((afterOrder * 3 + 1,
                 new Dictionary { ["type"] = "shop", ["data"] = shopData }));
    }

    var storyboards = journeyData.ContainsKey("storyboards")
                          ? journeyData["storyboards"].AsGodotArray()
                          : new Array();
    foreach (var storyboardVariant in storyboards) {
      var storyboardData = storyboardVariant.AsGodotDictionary();
      int order = storyboardData.ContainsKey("order")
                      ? storyboardData["order"].AsInt32()
                      : 0;
      items.Add((order * 3, new Dictionary { ["type"] = "storyboard",
                                             ["data"] = storyboardData }));
    }

    var forks = journeyData.ContainsKey("forks")
                    ? journeyData["forks"].AsGodotArray()
                    : new Array();
    foreach (var forkVariant in forks) {
      var forkData = forkVariant.AsGodotDictionary();
      int afterOrder = forkData.ContainsKey("after_order")
                           ? forkData["after_order"].AsInt32()
                           : 0;
      items.Add((afterOrder * 3 + 2,
                 new Dictionary { ["type"] = "fork", ["data"] = forkData }));
    }
  ...
  // Returns the current round's data dict. Empty if current item is a fork or
  // sequence is done.
  ...
  // Returns the current storyboard's data dict. Empty if current item is not a
  // storyboard.
  ...
  public void ResolveFork(int pathIndex) {
    ...
    // Record this choice in the play log so the end screen can show the path
    // taken. Depth is captured BEFORE incrementing so the header aligns with
    // where the fork appeared (e.g. depth 0 = top-level, depth 1 = inside
    // another fork's path).
    _playLog.Add(new Dictionary {
      ["type"] = "fork_choice",
      ["fork_title"] =
          forkData.ContainsKey("title") ? forkData["title"].AsString() : "",
      ["path_name"] = chosen.ContainsKey("name") ? chosen["name"].AsString()
                                                 : "Path " + (pathIndex + 1),
      ["path_index"] = pathIndex,
      ["depth"] = _forkDepth,
    });

    var chosenRounds = chosen.ContainsKey("rounds")
                           ? chosen["rounds"].AsGodotArray()
                           : new Array();
    var chosenShops = chosen.ContainsKey("shops")
                          ? chosen["shops"].AsGodotArray()
                          : new Array();
    var chosenStoryboards = chosen.ContainsKey("storyboards")
                                ? chosen["storyboards"].AsGodotArray()
                                : new Array();
    var chosenForks = chosen.ContainsKey("forks")
                          ? chosen["forks"].AsGodotArray()
                          : new Array();

    // Interleave path rounds, shops, storyboards, and nested forks by the same
    // sort-key scheme as BuildSequence so authoring order is preserved on
    // resolution.
    ...
    foreach (var chosenRound in chosenRounds) {
      ...
      int order =
          roundData.ContainsKey("order") ? roundData["order"].AsInt32() : 0;
      subItems.Add((order * 3, new Dictionary { ["type"] = "round",
                                                ["data"] = roundData }));
    }
    foreach (var chosenStoryboard in chosenStoryboards) {
      ...
      int order = storyboardData.ContainsKey("order")
                      ? storyboardData["order"].AsInt32()
                      : 0;
      subItems.Add((order * 3, new Dictionary { ["type"] = "storyboard",
                                                ["data"] = storyboardData }));
    }
    foreach (var chosenShop in chosenShops) {
      var shopData = chosenShop.AsGodotDictionary();
      int afterOrder = shopData.ContainsKey("after_order")
                           ? shopData["after_order"].AsInt32()
                           : 0;
      subItems.Add((afterOrder * 3 + 1,
                    new Dictionary { ["type"] = "shop", ["data"] = shopData }));
    }
    foreach (var chosenFork in chosenForks) {
      var chosenForkData = chosenFork.AsGodotDictionary();
      int afterOrder = chosenForkData.ContainsKey("after_order")
                           ? chosenForkData["after_order"].AsInt32()
                           : 0;
      subItems.Add(
          (afterOrder * 3 + 2,
           new Dictionary { ["type"] = "fork", ["data"] = chosenForkData }));
    }
    ...
    _sequence.Insert(_seqIndex + subItems.Count,
                     new Dictionary { ["type"] = "fork_end" });
    _forkDepth++;
  ...
  public void Advance() {
    ...
    // This correctly handles back-to-back sentinel runs when nested forks end
    // together.
  ...
  // fork_end entries are internal bookkeeping and must not be counted as real
  // items.
  public bool IsLastRound() {
    for (int i = _seqIndex + 1; i < _sequence.Count; i++) {
      string t = _sequence[i].ContainsKey("type")
                     ? _sequence[i]["type"].AsString()
                     : "";
      if (t != "fork_end")
        return false;
    }
  ...
  // Count of round-type items currently in the sequence (grows after fork
  // resolution).
  public int TotalRounds() => _sequence.Count(item => item["type"].AsString() ==
                                                      "round");
  ...
  public void LoadFromSave(Dictionary journeyData, Dictionary saveData) {
    Journey = journeyData;
    _seqIndex = saveData.ContainsKey("sequence_index")
                    ? saveData["sequence_index"].AsInt32()
                    : 0;
    _forkDepth = saveData.ContainsKey("fork_depth")
                     ? saveData["fork_depth"].AsInt32()
                     : 0;
    _playLog.Clear();
    ...
    if (_seqIndex < 0)
      _seqIndex = 0;
    if (_seqIndex > _sequence.Count)
      _seqIndex = _sequence.Count;
  }

  // Called by GameLoop after each round ends (before ScoreService.EndRound).
  // roundName and lengthMs are passed explicitly from GDScript (where
  // Dictionary access is known to work) to avoid C# String/StringName
  // key-lookup mismatches.
