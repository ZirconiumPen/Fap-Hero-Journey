...
func show_insert_popup(
	overlay: Control, graph: Control, arr: Array, insert_idx: int, screen_pos: Vector2
) -> void:
	...
	var specs: Array = [
		...
		{
			"label": "◈ STORYBOARD",
			"color": UITheme.STORYBOARD,
			"item": JourneyData.new_item("storyboard")
		},
...
func show_journey_info_panel() -> void:
	...
	bulk_hint.add_theme_color_override(
	...
	if not _owner._clipboard_items.is_empty():
		...
		UITheme.style_button(paste_btn, UITheme.AMBER)
		paste_btn.pressed.connect(
			func() -> void: _owner._paste_clipboard_into(_owner._items, _owner._items.size())
		)
...
func _make_tag_toggle(tag_def: Dictionary) -> Button:
	...
	off_style.corner_radius_top_left = 6
	off_style.corner_radius_top_right = 6
	off_style.corner_radius_bottom_left = 6
	off_style.corner_radius_bottom_right = 6
...
func _build_side_panel_editor(
	container: VBoxContainer, item: Dictionary, arr: Array, idx: int, graph: Control
) -> void:
	...
	var reselect: Callable = func(new_idx: int) -> void: graph.select_item(arr, new_idx)
...
func _side_action_row(arr: Array, idx: int, graph: Control, reselect: Callable) -> Control:
	...
	var dn_btn: Button = UITheme.make_icon_btn(
		"↓ MOVE DOWN", idx == arr.size() - 1, UITheme.PURPLE_MID
	)
...
func _make_side_storyboard_editor(
	arr: Array, idx: int, graph: Control, reselect: Callable
) -> Control:
...
func _make_fork_compact_editor(arr: Array, idx: int, graph: Control, reselect: Callable) -> Control:
	...
	desc_edit.text_changed.connect(func(val: String) -> void: arr[idx]["description"] = val)
	...
	if resolution == "conditional":
		...
		def_dd.item_selected.connect(func(i: int) -> void: arr[idx]["default_path"] = i)
...
func _make_path_editor_block(
	fork: Dictionary, paths_arr: Array, pi: int, graph: Control, reselect: Callable
) -> Control:
	...
	UITheme.style_line_edit(name_edit)
	name_edit.text_changed.connect(func(val: String) -> void: paths_arr[pi]["name"] = val)
	...
	UITheme.style_line_edit(desc_edit)
	desc_edit.text_changed.connect(func(val: String) -> void: paths_arr[pi]["description"] = val)
